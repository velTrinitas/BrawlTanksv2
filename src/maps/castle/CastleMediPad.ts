 ...
 * per mapa), art = heraldyczna tarcza z sercem na granitowym daisie (paleta Zielonej Doliny).
 *
...
function getPlatformTex(): PIXI.Texture {
    ...
    // Cien pod daisem (SE, jak wszystko w Zielonej Dolinie)
    c.fillStyle = 'rgba(10,20,12,0.45)';
    c.beginPath(); c.ellipse(cx + 3, cy + 6, 46, 40, 0, 0, Math.PI * 2); c.fill();

    // Granitowy dais (osmiokat) z jasnym wiekiem — fake 3D: dolna krawedz ciemniejsza
    c.fillStyle = '#5f656a';
    ...
    // Heraldyczna tarcza (szkarlat, zloty obrys) — symbol: serce = leczenie (jeden symbol = jedna mechanika)
    const sw = 40, sh = 46, sx = cx - sw / 2, sy = cy - sh / 2 - 2;
    c.fillStyle = '#7a1119';
    c.beginPath(); c.moveTo(sx + 2, sy + 2); c.lineTo(sx + sw + 2, sy + 2); c.lineTo(sx + sw + 2, sy + sh * 0.6 + 2); c.quadraticCurveTo(sx + sw / 2 + 2, sy + sh + 4, sx + 2, sy + sh * 0.6 + 2); c.closePath(); c.fill();
    const g = c.createLinearGradient(sx, sy, sx + sw, sy + sh);
    g.addColorStop(0, '#d7404a'); g.addColorStop(1, '#b3202a');
    c.fillStyle = g;
    c.beginPath(); c.moveTo(sx, sy); c.lineTo(sx + sw, sy); c.lineTo(sx + sw, sy + sh * 0.6); c.quadraticCurveTo(sx + sw / 2, sy + sh + 2, sx, sy + sh * 0.6); c.closePath(); c.fill();
    c.strokeStyle = '#e0b53c'; c.lineWidth = 2.5; c.stroke();
    // serce
    c.fillStyle = '#5effa0';
    c.beginPath();
    c.moveTo(cx, cy + 12);
    c.bezierCurveTo(cx - 18, cy - 2, cx - 10, cy - 18, cx, cy - 8);
    c.bezierCurveTo(cx + 10, cy - 18, cx + 18, cy - 2, cx, cy + 12);
    c.closePath(); c.fill();
    c.fillStyle = '#c6ffdd';
    c.beginPath(); c.ellipse(cx - 5, cy - 8, 3, 2, -0.5, 0, Math.PI * 2); c.fill();

    // Mech na granicie
    c.fillStyle = 'rgba(95,138,62,0.6)';
